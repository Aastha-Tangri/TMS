...
class DocumentController < ApplicationController
  ...
  def document
    ...
    CSV.parse(@file_content) do |row|
      @header=row if row_index == 0
      @header.reject!(&:blank?)
      @header.collect! {|i| i.downcase}
      if @header.include?(params[:encrypt_field]) == true
        ...
        flash[:notice] = "Emails have been sent successfully!"
      else
        flash[:notice] = "Encryption field is missing in the CSV"
      end
    end
    redirect_to "/document/index"
  end
